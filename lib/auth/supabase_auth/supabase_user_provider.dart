...
class ThredtestSupabaseUser extends BaseAuthUser {
  ThredtestSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> thredtestSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ThredtestSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
